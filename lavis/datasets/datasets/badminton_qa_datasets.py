...
#         images_list, text_inputs,qformer_instruction, text_outputs, answers, qids, chunks, impossibles, question_types = ([] for _ in range(9))
#         for s in samples:
...
#             question_types.append(s["question_type"])
...
#     def collater(self, samples):
#         images_list, text_inputs,qformer_instruction, qids, chunks, impossibles, answers, question_types = ([] for _ in range(8))
...
# ==============================================================================
# (貼上上面定義的 MULTI_TASK_INSTRUCTIONS 字典於此)
# ==============================================================================
# 通用的思考與輸出格式要求
...
class BadmintonQADataset(BaseDataset, __BadmintonMixin):
    """
    Dataset for chunk-based badminton QA. (TRAIN)
    """
    def __init__(self, vis_processor, text_processor, vis_root: str, ann_paths: list, train_samples_portion="all"):
        super().__init__(vis_processor, text_processor, vis_root, ann_paths=[])
        ann_path = ann_paths[0]
        with open(ann_path, 'r', encoding='utf-8') as f:
            self.annotation = json.load(f)
    ...
    def get_text_input(ann: dict) -> str:
        """
        <<<--- 核心修改區域 ---<<<
        Dynamically select instruction based on the task.
        """
        task = ann.get("task", "default")
        instruction_template = MULTI_TASK_INSTRUCTIONS.get(task, MULTI_TASK_INSTRUCTIONS["default"])
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "question":      ann["question"],
            "question_type": ann.get("question_type", "unknown"),
            "task":          ann.get("task", "unknown"),
        }

    # collater 方法保持不變 (與上一回答中提供的版本相同)
    def collater(self, samples):
        images_list, text_inputs, qformer_instruction, text_outputs, answers, qids, chunks, impossibles, question_types, tasks = ([] for _ in range(10))
        for s in samples:
            images_list.append(s["images"])
            text_inputs.append(s["text_input"])
            qformer_instruction.append(s["Qformer_instruction"])
        ...
        batch_images = torch.stack(padded, dim=0) if padded else torch.empty(0)
        clip_mask = torch.stack(clip_masks, dim=0) if clip_masks else torch.empty(0)

        return {
            "images": batch_images, "clip_mask": clip_mask, "text_input": text_inputs,
            "Qformer_instruction": qformer_instruction, "text_output": text_outputs, "answer": answers,
            "question_id": qids, "chunk_id": chunks, "is_impossible": impossibles,
            "question_type": question_types, "task": tasks,
        }


class BadmintonQAEvalDataset(VQAEvalDataset, __BadmintonMixin):
    """
    Evaluation dataset for chunk-based badminton QA. (VAL/TEST)
    """
    def __init__(self, vis_processor, text_processor, vis_root: str, ann_paths: list):
        super().__init__(vis_processor, text_processor, vis_root, ann_paths=[])
        ann_path = ann_paths[0]
        with open(ann_path, 'r', encoding='utf-8') as f:
            self.annotation = json.load(f)
    ...
    def get_text_input(ann: dict) -> str:
        """
        <<<--- 核心修改區域 ---<<<
        Dynamically select instruction based on the task. (Same as training set)
        """
        task = ann.get("task", "default")
        instruction_template = MULTI_TASK_INSTRUCTIONS.get(task, MULTI_TASK_INSTRUCTIONS["default"])
    ...
    # collater 方法保持不變 (與上一回答中提供的版本相同)
    def collater(self, samples):
        images_list, text_inputs, qformer_instruction, qids, chunks, impossibles, answers, question_types, tasks = ([] for _ in range(9))
        for s in samples:
            images_list.append(s["images"])
            text_inputs.append(s["text_input"])
            qformer_instruction.append(s["Qformer_instruction"])
        ...
        return {
            "images": batch_images, "clip_mask": clip_mask, "text_input": text_inputs,
            "Qformer_instruction": qformer_instruction, "question_id": qids, "chunk_id": chunks,
            "is_impossible": impossibles, "answer": answers, "question_type": question_types,
            "task": tasks,
        }