...
def bipartite_soft_matching(
    ...
    def merge(x: torch.Tensor, mode="mean") -> torch.Tensor:
        ...
        # Use scatter_add and then reduce
        dst.scatter_add_(-2, dst_idx.expand(n, r, c), src)     # [n, t2, c]
        if mode == "mean":
            count = torch.ones_like(src).scatter_add_(-2, dst_idx.expand(n, r, c), torch.ones_like(src))
            dst = dst / count
