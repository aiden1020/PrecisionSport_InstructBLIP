...
from lavis.models.tc_clip_encoder.clip.model_utils import LayerNorm
from lavis.models.tc_clip_encoder.clip.transformer import Transformer

from lavis.models.tc_clip_encoder.tome.utils import parse_r
...
class TCVisionTransformer(nn.Module):
    ...
    def get_num_layer(self, var_name=""):
        if var_name in {"class_embedding", "positional_embedding", "conv1"}:
            return 0  # 屬於輸入層或額外的嵌入
        
        elif var_name.startswith("ln_pre") or var_name.startswith("ln_post"):
            return 0  # LayerNorm 屬於標準化層，影響較小

        elif var_name.startswith("proj"):
            return self.num_layers  # 投影層通常設為最高層

        elif var_name.startswith("transformer.blocks"):
            parts = var_name.split('.')
            if len(parts) > 2 and parts[2].isdigit():  
                return int(parts[2]) + 1  # 解析 Transformer block 的層數

        return self.num_layers  # 其他情況回傳最高層
